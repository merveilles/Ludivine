...
class Answer
    ...
    def lookup
        ...
        if thoughts.length < 1 
            return "I don't know what *"+query.sub("my ","your ")+"* is, but you can teach me about it \bby typing `remember that "+query+" is ` followed by a short definition."
        end

        if query.split(" ")[0] == "my"
            thoughts.each do |known|
                if known[0] != @username then next end
                return "Your "+query.sub("my ","")+" is *"+known[2]+"*."
            end
            return "I don't know what your "+query.sub("my ","")+" is.. "
        end
